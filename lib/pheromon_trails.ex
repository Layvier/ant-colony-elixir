defmodule PheromonTrails do
  ...
  def handle_call({:new_ant_path, path, cost}, _from, state) do
    new_pheromons = state[:pheromons] |> evaporate_pheromons |> rec_add_next_pheromon(path, cost)
  ...
  def evaporate_pheromons(pheromons) do
    SymetricGraph.apply_to_all(pheromons, fn (pheromon_val) -> evaporate(pheromon_val) end)
  end
  ...
  def set_pheromon_value(pheromons, i, j, value) do
    ...
    SymetricGraph.set_value(pheromons, i, j, current_pheromon_value + (1 - current_pheromon_value)*(1 / value))
    # new_pheromon_value = 
